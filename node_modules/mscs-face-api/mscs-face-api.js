...
function MSCSFaceApi(key, server) {
    ...
    function createPerson(personGroupId, name, userData) {
        return new Promise((resolve, reject) => {
            axios({
				...
				console.log(res.data);
				global.pIdValue = res.data.personId;
    ...
    function detectFace(image) {
        return new Promise((resolve, reject) => {
            axios({
                method: "post",
                url: `${DETECT}?returnFaceId=true&returnFaceAttributes=emotion`,
                headers: HEADERS,
                data: {
                    "url": image
                }
                ...
                resolve(res.data);
				console.log('detect '+res.data[0].faceId);
				var faceIds = res.data[0].faceId;
				console.log(res.data);
				config2.faceDetection.faceIds[0] = faceIds;

				console.log('config.faceDetection.faceIds[0] 에 들어간 아이디 값 '+config2.faceDetection.faceIds[0]);
				config2.faceDetection.emotion.anger =  res.data[0].faceAttributes.emotion.anger;
				config2.faceDetection.emotion.contempt =  res.data[0].faceAttributes.emotion.contempt;
				config2.faceDetection.emotion.disgust =  res.data[0].faceAttributes.emotion.disgust;
				config2.faceDetection.emotion.fear =  res.data[0].faceAttributes.emotion.fear;
				config2.faceDetection.emotion.happiness =  res.data[0].faceAttributes.emotion.happiness;
				config2.faceDetection.emotion.neutral =  res.data[0].faceAttributes.emotion.neutral;
				config2.faceDetection.emotion.sadness =  res.data[0].faceAttributes.emotion.sadness;
				config2.faceDetection.emotion.surprise =  res.data[0].faceAttributes.emotion.surprise;
				console.log(config2.faceDetection);

				//identifyFace('ditmirrorgroup', config.faceDetection.faceIds, 0.6);
            }).catch(function (err) {
                reject(err.response.data.error);
				console.log(err);
            })
     ...
     */
    function identifyFace(personGroupId,  confidenceThreshold) {
		var faceIds = config2.faceDetection.faceIds;
		console.log(config2.faceDetection.faceIds+"을 비교합니다");
        ...
        return new Promise((resolve, reject) => {
            Promise.all(all)
                .then((res) => {
                    ...
                    resolve(identify);
					console.dir("identify:"+identify);
          console.dir(identify);
					config2.faceDetection.confidence = identify[0].candidates[0].confidence;
          console.log(config2.faceDetection.confidence);
					global.personId = identify[0].candidates[0].personId;

					if(config2.faceDetection.confidence>0.5){
						$scope.face = "얼굴인식 완료";
						if(responsiveVoice.voiceSupport()) {
							responsiveVoice.speak("얼굴 인식이 완료 되었습니다.","Korean Female");
							}
					}
                })
